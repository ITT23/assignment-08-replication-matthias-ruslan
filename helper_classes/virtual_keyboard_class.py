    ...
    def init_keyboard(self):
        ...
        for i in range(0, len(rows)):
            ...
            for key in keys:
                ...
                y_pos = (Config.KEYBOARD_HEIGHT - (key_height * row_position)) - row_position
                key_shape = shapes.Rectangle(int(x_pos), y_pos, key_width,
                                             key_height,
                                             color=(191, 191, 191, 255), batch=self.key_batch)
                key_text = pyglet.text.Label(keys[key][symbol_key], font_size=12, color=(0, 0, 0, 255), batch=self.key_batch)
                text_x = x_pos + (key_width - key_text.content_width) // 2
                text_y = y_pos + (key_height - key_text.content_height) // 2
                key_text.x = text_x
                key_text.y = text_y
                self.key_shapes.append(
                    {"shape": key_shape, "label": key_text, "x": int(x_pos), "y": y_pos,
                     'width': key_width, 'height': key_height,
                     'input': keys[key][input_key], 'symbol': keys[key][symbol_key]})
