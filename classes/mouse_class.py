...
class Mouse():
    ...
    def gyro_set_pos(self, new_x_pos, new_y_pos):
        if new_x_pos < 0:
            self.pos_x = -1
        else:
            self.pos_x = 1
    ...
    def move(self):
        pyautogui.moveRel(self.pos_x * Config.MOUSE_MOVEMENT_SCALING,
                          self.pos_y * Config.MOUSE_MOVEMENT_SCALING)  # scaled by 10
